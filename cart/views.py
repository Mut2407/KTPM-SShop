...
def add_cart(request, product_id):
    ...
    # 1. CHECK STOCK: Chặn ngay nếu sản phẩm đã hết hàng
    if product.stock <= 0:
        ...
        if is_cart_item_exist:
            ...
            if product_variation in ex_var_list:
                ...
                if item.quantity >= product.stock:
                    messages.warning(request, f'Only {product.stock} items left in stock.')
                else:
                    item.quantity += 1
                    item.save()
            else:
...
def cart(request, total_price=0, quantity=0, cart_items=None):
    ...
    context = {
        ...
        'vat': tax,
        'order_total': float(grand_total) + 15.00,
        'handing': 15.00,
    }
        ...
        try:
            ...
            if action == 'increase':
                ...
                if cart_item.quantity >= cart_item.product.stock:
                    return JsonResponse({'status': 'error', 'message': f'Only {cart_item.product.stock} items left in stock.'})
                cart_item.quantity += 1
