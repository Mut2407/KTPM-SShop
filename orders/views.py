    ...
    if insufficient:
        for product, stock in insufficient:
            if stock == 0:
                ...
                messages.error(request, f'{product.name}: Only {stock} items left. Please reduce quantity.')
...
def payment(request, total=0, quantity=0):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Information is not valid. Please check your input.')
...
def cod_payment(request):
    if request.method == 'POST':
        try:
            ...
            # --- 3. CHECK STOCK LẠI LẦN NỮA CHO CHẮC ---
            cart_items = CartItem.objects.filter(user=request.user)
            resp = _validate_cart_stock_or_redirect(request, cart_items)
            if resp: 
                return JsonResponse({'status': 'error', 'message': 'Some items are out of stock. Please check your cart.'})
            # -------------------------------------------------
